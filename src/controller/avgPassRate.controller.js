...
// Controller to calculate the average pass rate for a RouteCentre
export const calculateAveragePassRateForRouteCentre = async (req, res, next) => {
    ...
    try {
        ...
        if (result.length > 0) {
            ...
            return res.status(404).json({ status: false, message: "No attempted tests found for this RouteCentre." });
        }
    } catch (error) {
        next(error);
    }
